function [F,Ud] = F_qpoms_CR3BP_matrix_Jacobi_constant_fix(Z,zpo,Ud,p,C_periodic)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Compute the error vector in calculating QPO by using GMOS method

%%% input
% Z :current state variables [X;T;rho]
...
% vector field (M=1は先頭に，M=15は最後に格納している)
fT = zeros(d*N,1);
for i = 1:M
    UdT = Ud{5,2}(:,i);
    for j=1:N
...
%% FIX Jacobi Constant (全てのMに関して)
sC=0;
for i = 1:M
    for j = 1:N
        vec_begin = d*N*(i-1)+d*(j-1)+1;
        vec_end = d*N*(i-1)+d*j;
        C = fun_Jacobi_const_CR3BP(Z(vec_begin:vec_end, 1), mu);
        sC = sC + C;
    end
end
s0z = sC/N/M-C_periodic;
